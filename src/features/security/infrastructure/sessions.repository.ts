...
export class SessionsRepository {
    ...
    async updateExistSession(deviceId: string, updateModel: SessionUpdateModel) {
        try {
            const session: SessionDocument = await this.sessionsModel.findOne({deviceId: deviceId});
    ...
    async deleteSessionsExpectCurrent(userId: string, deviceId: string) {
        try {
            await this.sessionsModel.deleteMany({$and: [{userId: userId}, {deviceId: {$not: {$eq: deviceId}}}]});
        } catch {
            throw new Error()
        }
    }
