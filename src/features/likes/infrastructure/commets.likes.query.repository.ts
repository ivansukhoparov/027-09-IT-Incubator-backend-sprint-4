...
export class CommentsLikesQueryRepository{
    ...
    async getLikes(commentId:string, userId?:string):Promise<LikesInfoType>{
        ...
        const likesCount =  await this.commentLikesModel.countDocuments({$and: [{commentId: commentId}, {status:"Like"}]});
        const dislikesCount =  await this.commentLikesModel.countDocuments({$and: [{commentId: commentId}, {status:"Dislike"}]});
        return 	{
