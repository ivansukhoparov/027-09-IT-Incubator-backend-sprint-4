...
export class PostsLikesQueryRepository{
    ...
    async getLikes(postId:string, userId?:string):Promise<PostsLikesInfoType>{
        ...
        const likesCount =  await this.postsLikesModel.countDocuments({$and: [{postId: postId}, {status:"Like"}]});
        const dislikesCount =  await this.postsLikesModel.countDocuments({$and: [{postId: postId}, {status:"Dislike"}]});
        const newestLikes:Array<PostsLikesDocument> = await this.postsLikesModel.find({$and: [{postId: postId}, {status: "Like"}]}).sort({"addedAt": "desc"}).limit(3).lean();
